...
<html>
	...
	<body>
		...
		<form name="f" id="contactForm" action="tfPlusMemberProfileUpdatePro" method="post" enctype="multipart/form-data">
			<fieldset>
				<table class="jjm494">
					<tr>
						<th scope="row">이름</th>
						<td>
							<label>${memberProfileUpdate.mProfileName}</label>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<c:forTokens items="${testVar}" delims="," var="value">
								<c:if test="${memberProfileUpdate.mCheckOption == value}">
					...
					<tr>
						...
						<td>
							<textarea  name="profileContents"  id="comments" rows="2" cols="6" class="form-poshytip" title="한줄평을 남기세요">${memberProfileUpdate.mProfileContents}</textarea>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div style="text-align:center;">
								...
								<div id="img_box">
									<img src="${memberProfileUpPath}/${memberProfileUpdate.mPhoto}" width="300px" height="100px"/>
								</div>
							</div>
						</td>
					</tr>
					<tr>
						<th scope="row" colspan="2" align="center">
							...
							<!-- 히든으로 넘어갈 정보들 -->
								<input type="hidden" value="${memberProfileUpdate.mProfileName}" name="profileName" id="to"/>
								<input type="hidden" value="${memberProfileUpdate.mProfileNum}" name="profileNum" id="to"/>
								<input type="hidden" value="${sessionScope.memberDTO.id}" name="profileId" id="to" />
								<input type="hidden" value="${memberProfileUpdate.mPhoto}" name="photoOrg" id="to" />
							<!-- 히든으로 넘어갈 정보들 -->
